...
export class CleanupCodeExecutor {
	public constructor(private readonly output: vscode.OutputChannel, private readonly statusBarItem: vscode.StatusBarItem) {}
	...
	public executeCleanupCode(filePath: string, options: string[] = []): void {
		this.output.appendLine(`Cleanup Code command is running for '${filePath}'...`);

		const cp = spawn(CLEANCODE_COMMAND, [filePath, ...options], { shell: true });
	...
	public run(file?: string) {
		if (file) {
			...
			if (projFile) {
				...
				this.executeCleanupCode(projFile);
			}
		} else {
			selectSolutionFile((filePath) => {
				...
				vscode.window
					.showQuickPick(['No. Do not change my codes.', 'Yes. Cleanup my codes.'], {
